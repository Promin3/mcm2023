...
dataframe = pd.read_excel('A/附件.xlsx')
...
# eta_at
d_HR_s = np.array([np.sqrt(x ** 2 + y ** 2 + (h_tower - h_mirror) ** 2) for x, y in mirror_pos])
...
# eta_cos
d_n = np.array(
    [np.sqrt(h_tower ** 2 + mirror_pos[i][0] ** 2 + mirror_pos[i][1] ** 2) for i in range(mirror_pos.shape[0])])
x_s = mirror_pos[:, 0]
y_s = mirror_pos[:, 1]
